...
add_action( 'hoot_dynamic_cssrules', 'nevark_dynamic_cssrules', 5 );
 ...
 * @return array|string
 */
if ( !function_exists( 'nevark_user_style' ) ) :
function nevark_user_style( $key = false ){
	static $styles;
	...
	if ( empty( $styles ) ) {
		...
		$styles['logo_custom']          = apply_filters( 'nevark_logo_custom_text', hoot_sortlist( hoot_get_mod( 'logo_custom' ) ) );
		...
		$styles['smallwidgetmargin']    = ( intval( $styles['widgetmargin'] ) ) ?
		                                    ...
		                                    false;
		$styles = apply_filters( 'nevark_user_style', $styles );
	}
	if ( $key )
 ...
 * @access public
 */
function nevark_dynamic_cssrules() {
	...
	elseif ( 'heading' == $headings_fontface )
		$headingproperty['font-family'] = array( '"Oranienbaum", serif' );
	...
	elseif ( 'heading' == $logo_fontface )
		$logoproperty['font-family'] = '"Oranienbaum", serif';
	...
	hoot_add_css_rule( array(
						'selector' => '#site-logo.accent-typo',
						'property' => 'border-color',
						'value'    => $accent_color,
						'idtag'    => 'accent_color',
					) );
	hoot_add_css_rule( array(
						'selector' => '#site-logo.invert-accent-typo',
						'property' => 'border-color',
						'value'    => $accent_font,
						'idtag'    => 'accent_font',
					) );
	...
	elseif ( 'heading' == $headings_fontface )
		$sitetitleheadingfont = '"Oranienbaum", serif';
	elseif ( 'heading2' == $headings_fontface )

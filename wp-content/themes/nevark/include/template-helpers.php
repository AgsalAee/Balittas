 ...
 * @return void
 */
if ( !function_exists( 'nevark_branding' ) ):
function nevark_branding() {
	?>
	<div <?php hoot_attr( 'branding' ); ?>>
		<div id="site-logo" class="<?php
			...
			elseif ( hoot_get_mod('logo_background_type') == 'invert' )
 ...
 * @return void
 */
if ( !function_exists( 'nevark_logo' ) ):
function nevark_logo() {

	$display = '';
	$nevark_logo = hoot_get_mod( 'logo' );
	...
	if ( 'text' == $nevark_logo || 'custom' == $nevark_logo ) {
		$display .= nevark_get_text_logo( $nevark_logo, $tag_h1, $tag_h2 );
	} elseif ( 'mixed' == $nevark_logo || 'mixedcustom' == $nevark_logo ) {
		$display .= nevark_get_mixed_logo( $nevark_logo, $tag_h1, $tag_h2 );
	} elseif ( 'image' == $nevark_logo ) {
		$display .= nevark_get_image_logo( $nevark_logo, $tag_h1, $tag_h2 );
	}

	echo wp_kses( apply_filters( 'nevark_logo', $display, $nevark_logo, $tag_h1, $tag_h2 ), hoot_data( 'hootallowedtags' ) );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $nevark_logo text|custom
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'nevark_get_text_logo' ) ):
function nevark_get_text_logo( $nevark_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$title_icon = hoot_sanitize_fa( hoot_get_mod( 'site_title_icon', NULL ) );

	$class = $id = 'site-logo-' . esc_attr( $nevark_logo );
	$class .= ( $title_icon ) ? ' site-logo-with-icon' : '';
	$class .= ( 'text' == $nevark_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
			$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
				...
				$title = ( 'custom' == $nevark_logo ) ? nevark_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
				$display .= apply_filters( 'nevark_site_title', $title );
			$display .= "</a>";
		$display .= "</{$tag_h1}>";
	...
	return apply_filters( 'nevark_get_text_logo', $display, $nevark_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $nevark_logo mixed|mixedcustom
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'nevark_get_mixed_logo' ) ):
function nevark_get_mixed_logo( $nevark_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$has_logo = has_custom_logo();

	$class = $id = 'site-logo-' . esc_attr( $nevark_logo );
	$class .= ( !empty( $has_logo ) ) ? ' site-logo-with-image' : '';
	$class .= ( 'mixed' == $nevark_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= '<div class="site-logo-mixed-text">';
			...
			$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
				$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
					$title = ( 'mixedcustom' == $nevark_logo ) ? nevark_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
					$display .= apply_filters( 'nevark_site_title', $title );
				$display .= "</a>";
			$display .= "</{$tag_h1}>";
	...
	return apply_filters( 'nevark_get_mixed_logo', $display, $nevark_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $nevark_logo
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'nevark_get_image_logo' ) ):
function nevark_get_image_logo( $nevark_logo = 'image', $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$has_logo = has_custom_logo();
	...
	return apply_filters( 'nevark_get_image_logo', $display, $nevark_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'nevark_get_custom_text_logo' ) ):
function nevark_get_custom_text_logo() {
	$title = '';
	$logo_custom = apply_filters( 'nevark_logo_custom_text', hoot_sortlist( hoot_get_mod( 'logo_custom' ) ) );
	...
	return apply_filters( 'nevark_get_custom_text_logo', $title, $logo_custom );
}
endif;
 ...
 * @return void
 */
if ( !function_exists( 'nevark_header_aside' ) ):
function nevark_header_aside() {
 ...
 * @return void
 */
if ( !function_exists( 'nevark_secondary_menu' ) ):
function nevark_secondary_menu( $location ) {
 ...
 * @return void
 */
function nevark_nav_menu_toplevel_items( $theme_location = 'hoot-secondary-menu' ) {
 ...
 * @return string
 */
if ( !function_exists( 'nevark_menu_description' ) ):
function nevark_menu_description( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'nevark_menu_description', 5, 4 );
 ...
 * @return void
 */
if ( !function_exists( 'nevark_add_custom_title_content' ) ):
function nevark_add_custom_title_content( $location = 'pre', $context = '' ) {

	$pre_title_content_post = apply_filters( 'nevark_pre_title_content_post', '', $location, $context );
	if ( ( $location == 'pre' && !$pre_title_content_post ) ||
		 ( $location == 'post' && $pre_title_content_post ) ) : 

		$pre_title_content = apply_filters( 'nevark_pre_title_content', '', $location, $context );
		if ( !empty( $pre_title_content ) ) :

			$pre_title_content_stretch =  apply_filters( 'nevark_pre_title_content_stretch', '', $location, $context ); ?>
 ...
 * @return void
 */
if ( !function_exists( 'nevark_display_404_content' ) ):
function nevark_display_404_content() {
	echo esc_html( __( 'Apologies, but no entries were found.', 'nevark' ) );
}
endif;
add_action( 'nevark_404_content', 'nevark_display_404_content', 5 );
 ...
 * @return void
 */
if ( !function_exists( 'nevark_footer_structure' ) ):
function nevark_footer_structure() {
 ...
 * @return int
 */
function nevark_get_footer_columns() {
	$footers = hoot_get_mod( 'footer' );
 ...
 * @return void
 */
if ( !function_exists( 'nevark_get_column_span' ) ):
function nevark_get_column_span( $col_width ) {
	$return = array();
	switch( $col_width ):
		case '100':
...
/**
 * Wrapper function for nevark_layout() to get the class names for current context.
 ...
 * @return string
 */
if ( !function_exists( 'nevark_layout_class' ) ):
function nevark_layout_class( $context ) {
	return nevark_layout( $context, 'class' );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'nevark_layout' ) ):
function nevark_layout( $context, $return = 'size' ) {
 ...
 * @access public
 */
if ( !function_exists( 'nevark_set_layout' ) ):
function nevark_set_layout() {
	...
	$sidebar = esc_attr( apply_filters( 'nevark_layout', $sidebar ) );

	// Save the layout for current view
	nevark_set_layout_span( $sidebar );
 ...
 * @access public
 */
if ( !function_exists( 'nevark_set_layout_span' ) ):
function nevark_set_layout_span( $sidebar ) {
	$spans = apply_filters( 'nevark_layout_spans', array(
		'none' => array(
			'content' => 9,
			'sidebar' => 0,
		),
 ...
 * @access public
 */
if ( !function_exists( 'nevark_thumbnail_size_contentwidth' ) ):
function nevark_thumbnail_size_contentwidth() {
	return 'span-' . nevark_layout( 'content' );
}
endif;
add_filter( 'hoot_thumbnail_size_contentwidth', 'nevark_thumbnail_size_contentwidth' );
 ...
 * @access public
 */
if ( !function_exists( 'nevark_titlearea_top' ) ):
function nevark_titlearea_top() {
	...
	if ( in_array( 'no-sidebar', $full ) ) {
		$sidebar_size = nevark_layout( 'primary-sidebar' );
		if ( empty( $sidebar_size ) )
			return apply_filters( 'nevark_titlearea_top', true, 'no-sidebar', $full );
	}

	/* For Posts */
	if ( is_singular( 'post' ) ) {
		if ( in_array( 'posts', $full ) )
			return apply_filters( 'nevark_titlearea_top', true, 'posts', $full );
		else
			return apply_filters( 'nevark_titlearea_top', false, 'posts', $full );
	}

	/* For Pages */
	if ( is_page() ) {
		if ( in_array( 'pages', $full ) )
			return apply_filters( 'nevark_titlearea_top', true, 'pages', $full );
		else
			return apply_filters( 'nevark_titlearea_top', false, 'pages', $full );
	}

	/* Default */
	if ( in_array( 'default', $full ) )
		return apply_filters( 'nevark_titlearea_top', true, 'default', $full );
	else
		return apply_filters( 'nevark_titlearea_top', false, 'default', $full );
 ...
 * @access public
 */
if ( !function_exists( 'nevark_loopmeta_header_img' ) ):
function nevark_loopmeta_header_img( $context, $display ) {
	...
	$img_id = apply_filters( 'nevark_loopmeta_header_img_id', $img_id, $context, $location, $view_id );
	$img_id = absint( $img_id );

	if ( ( $location == 'header' || $location == 'staticheader' || $location == 'staticheader-nocrop' ) && !empty( $img_id ) ) {
		$img_src = wp_get_attachment_image_src( $img_id, apply_filters( "nevark_{$context}_imgsize", 'full', 'header' ) );
		$image = '';
		if ( !empty( $img_src[0] ) ) {
			if ( $location == 'header' ) {
				$wrap_attr = array(
 ...
 * @return void
 */
if ( !function_exists( 'nevark_ns_filter_avatar' ) ):
function nevark_ns_filter_avatar( $avatar, $id_or_email, $size, $default, $alt, $args ) {
 ...
 * @return void
 */
if ( !function_exists( 'nevark_post_prev_next_links' ) ):
function nevark_post_prev_next_links() {
	if ( hoot_get_mod( 'post_prev_next_links' ) ) {
		$display = true; $style = '';
	}
	if ( !empty( $display ) ) :
	?><div id="loop-nav-wrap" class="loop-nav"<?php echo $style; ?>><?php
		previous_post_link( '<div class="prev">' . __( 'Previous Post: %link', 'nevark' ) . '</div>', '%title' );
		next_post_link(     '<div class="next">' . __( 'Next Post: %link',     'nevark' ) . '</div>', '%title' );
	?></div><!-- .loop-nav --><?php
	endif;
}
endif;
 ...
 * @access public
 */
if ( !function_exists( 'nevark_jetpack_infinitescroll_render' ) ):
function nevark_jetpack_infinitescroll_render(){

 ...
 * @return void
 */
if ( !function_exists( 'nevark_woo_setup' ) ) :
function nevark_woo_setup() {
...
add_action( 'wp', 'nevark_woo_setup' );
 ...
 * @return void
 */
if ( !function_exists( 'nevark_woo_register_sidebars' ) ) :
function nevark_woo_register_sidebars() {
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-primary',
			'name'        => _x( 'Woocommerce Primary Sidebar', 'sidebar', 'nevark' ),
			'description' => __( 'The primary sidebar for woocommerce pages.', 'nevark' )
		)
	);
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-secondary',
			'name'        => _x( 'Woocommerce Secondary Sidebar', 'sidebar', 'nevark' ),
			'description' => __( 'The secondary sidebar for woocommerce pages (if you are using a 3 column layout with 2 sidebars).', 'nevark' )
		)
	);
}
endif;
add_action( 'widgets_init', 'nevark_woo_register_sidebars' );
 ...
 * @return array
 */
if ( !function_exists( 'nevark_woo_attr_sidebar' ) ) :
function nevark_woo_attr_sidebar( $attr, $context ) {
...
add_filter( 'hoot_attr_sidebar', 'nevark_woo_attr_sidebar', 11, 2 );
 ...
 * @return array
 */
if ( !function_exists( 'nevark_woo_layout' ) ) :
function nevark_woo_layout( $sidebar ) {
	...
	$forcenosidebar = apply_filters( 'nevark_woo_pages_force_nosidebar', true );
...
add_filter( 'nevark_layout', 'nevark_woo_layout' );
 ...
 * @param string $context
 */
if ( !function_exists('nevark_woo_meta_info_blocks_display') ) :
function nevark_woo_meta_info_blocks_display( $display, $context ) {
	if ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() )
		$display = array();
	return $display;
}
endif;
add_filter( 'hoot_meta_info', 'nevark_woo_meta_info_blocks_display', 10, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'nevark_woo_set_queried_object' ) ) :
function nevark_woo_set_queried_object( $q ){
	if ( $q->is_main_query() )
		$r = get_queried_object();
}
endif;
add_action( 'pre_get_posts', 'nevark_woo_set_queried_object', 9 );
 ...
 * @return void
 */
if ( !function_exists( 'nevark_woo_loop_hide_product_meta' ) ) :
function nevark_woo_loop_hide_product_meta() {

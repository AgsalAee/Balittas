...
function hoot_attr_body( $attr ) {
	...
	$class = apply_filters( 'nevark_default_body_class', 'nevark' );
...
function hoot_attr_sidebar( $attr, $context ) {
	...
	if ( $context ) {
		...
		if ( $sidebar_name ) {
			...
			$attr['aria-label'] = esc_attr( sprintf( _x( '%s Sidebar', 'sidebar aria label', 'nevark' ), $sidebar_name ) );
		}
	}
...
function hoot_attr_menu( $attr, $context ) {
	...
	if ( $context ) {
		...
		if ( $menu_name ) {
			...
			$attr['aria-label'] = esc_attr( sprintf( _x( '%s Menu', 'nav menu aria label', 'nevark' ), $menu_name ) );
		}
	}
...
function hoot_attr_entry_published( $attr ) {
	...
	$attr['title']    = get_the_time( _x( 'l, F j, Y, g:i a', 'post time format', 'nevark' ) );
...
function hoot_attr_comment_published( $attr ) {
	...
	$attr['title']    = get_comment_time( _x( 'l, F j, Y, g:i a', 'comment time format', 'nevark' ) );
	$attr['itemprop'] = 'datePublished';
